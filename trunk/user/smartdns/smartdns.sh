...
Read_ini () {
# [Read the port when the last successful startup, etc.]
...
Check_md5 () {
    ...
    if [ -s "$md5" ] ; then    # exist, then compare
        diff $md5 $new_md5 >/dev/null 2>&1
        [ $? -eq 1 ] && status=1    # different, store and update
    else
        status=1    # does not exist, store and update
    fi
...
Check_ss(){
    if [ -s /etc_ro/ss_ip.sh ] ; then
        if [ $(nvram get ss_enable) = 1 ] && [ $(nvram get ss_run_mode) = "router" ] && [ $(nvram get pdnsd_enable) = 0 ] ; then
            logger -t "SmartDNS" "The system detects that the SS mode is bypassing the mainland mode, and pdnsd is enabled, please adjust the SS resolution first to use SmartDNS + manual configuration mode! the program will exit..."
...
Get_sdns_conf () {
    ...
    do
        ...
        if  [ "$sdnss_enable" -eq 1 ] ; then
            ...
            if [ "$sdnss_ipset"x != x ] ; then
                # Call the check_ip_Addr function to check whether the ip is compliant
    ...
    if [ "$sdns_white" = "1" ] && [ -f "$chn_Route" ] ; then
        :>/tmp/whitelist.conf
        logger -t "SmartDNS" "Start working with whitelisted IP"
    ...
    if [ "$sdns_black" = "1" ] && [ -f "$chn_Route" ] ; then
        :>/tmp/blacklist.conf
        logger -t "SmartDNS" "Start processing blacklist IP"
...
Get_sdnse_conf () {
    # [Read the second server settings]
...
Check_ip_addr () {
    ...
    # IP address must be all numbers
    if [ $? -ne 0 ] ; then
        return 1
    fi
    ipaddr=$1
    ...
    for num in $a $b $c $d
    do
        if [ "$num" -gt 255 ] || [ "$num" -lt 0 ] ; then   # Each value must be between 0-255
            return 1
        fi
    done
    return 0
...
Start_AD () {
# [Download ad filter file]
    curl -s -o /tmp/sdnsadnew.conf --connect-timeout 10 --retry 3 $(nvram get sdns_adblock_url)
    if [ ! -f "/tmp/sdnsadnew.conf" ]; then
        logger -t "SmartDNS" "The advertisement filtering function is not enabled or the filtering address is invalid, the network is abnormal, etc. ！！！"
    else
        logger -t "SmartDNS" "Go to ad file download successfully Advertisement filtering is enabled..."
        if [ -f "/tmp/sdnsadnew.conf" ]; then
...
# 【】
    ...
    0)
        if [ $(nvram get adbyby_add) = 1 ] && [ "$hosts_type" != "dnsmasq" ]; then
            ...
            logger -t "SmartDNS" "DNS go to advertising rules: SmartDNS ⇒ DNSmasq"
            hosts_type="dnsmasq"
        fi
        ;;
    1)
        if [ "$hosts_type" != "SmartDNS" ] && [ "$action" = "start" ] ; then
            if [ "$sdns_port" = "53" ] || [ $(nvram get adbyby_add) = 1 ] || [ "$sdns_redirect" = "2" ] ; then
                ...
                /usr/bin/adbyby.sh switch
                logger -t "SmartDNS" "DNS go to advertising rules: DNSmasq ⇒ SmartDNS"
                hosts_type="SmartDNS"
            fi
        fi
        ;;
...
Change_dnsmasq () {
    # Delete related entries in DNSmasq configuration file to avoid duplication
    case $action in
    stop)
        ...
        if [ "$sdns_enable" = 0 ] ; then
            [ "$sdns_ported" = "53" ] && logger -t "SmartDNS" "DNSmasq domain name resolution (DNS) function enabled" 
            [ "$sdns_redirected" = "1" ] && logger -t "SmartDNS" "Delete DNSmasq upstream server：127.0.0.1:$sdns_ported" 
        fi
        ;;
    start)
        # When starting SmartDNS
        if [ "$sdns_port" = "53" ] ; then
            echo "port=0" >> "$dnsmasq_Conf"
            logger -t "SmartDNS" "The DNSmasq domain name resolution (DNS) function is turned off"
            if [ "$sdns_redirect" = "1" ] ; then
                nvram set sdns_redirect=0
                sdns_redirect=0
                logger -t "SmartDNS" "Therefore, automatically modify the redirection to: None" 
            fi
        fi
        if [ "$sdns_redirect" = "1" ] ; then
            echo "no-resolv" >> "$dnsmasq_Conf"
            echo "server=127.0.0.1#$sdns_port" >> "$dnsmasq_Conf"
            logger -t "SmartDNS" "As DNSmasq upstream server: 127.0.0.1:$sdns_port"
            if [ "$sdnse_enable" = 1 ] ; then
                logger -t "SmartDNS" "As DNSmasq upstream server: 127.0.0.1:$sdnse_port"
            fi
        fi
        ;;
    esac
...
Change_iptable () {
    ...
    stop)
        if [ "$sdns_redirected" = 2 ] ; then
            ...
            [ "$sdns_enable" = 0 ] && logger -t "SmartDNS" "Revert redirection $IPS4:$sdns_ported to xxx.xxx.xxx:53"
        fi
    ...
    start)
        if [ "$sdns_redirected" != 2 ] && [ "$sdns_redirect" = 2 ] ; then
            statu=1
            logger -t "SmartDNS" "Redirect xxx.xxx.xxx:53 to $IPS4:$sdns_port"
            if [ "$sdnse_enable" = 1 ] ; then
                logger -t "SmartDNS" "Redirect xxx.xxx.xxx:53 to $IPS4:$sdnse_port"
            fi
        fi
        ;;
    reset)
...
Start_smartdns () {
    ...
    if [ "$sdns_redirect" = 0 ] ; then
        logger -t "SmartDNS" "SmartDNS uses $sdns_port port"
        if [ "$sdnse_enable" = 1 ] ; then
            logger -t "SmartDNS" "SmartDNS uses $sdnse_port port"
        fi
    fi
    Change_iptable
    ...
    args=""
    logger -t "SmartDNS" "Create configuration file..."
    ...
    # Configuration file deduplication
    awk '!x[$0]++' "$smartdns_tmp_Conf" > "$smartdns_Conf"
    rm -f "$smartdns_tmp_Conf"
    if [ "$sdns_coredump" = "1" ] ; then
        args="$args -S"
    fi
    # Determine whether to restart the DNSmasq process by detecting whether the configuration file has changed
    if [ "$dnsmasq_md5" != $(md5sum  "$dnsmasq_Conf" | awk '{ print $1 }') ] ; then
        logger -t "SmartDNS" "Restarting DNSmasq process..."
        /sbin/restart_dhcpd >/dev/null 2>&1
        logger -t "SmartDNS" "DNSmasq process restarted..."
    fi
    # Start the smartdns process
    ...
    if [ "$smartdns_process"x = x ] ; then
        if [ "$hosts_type" = "SmartDNS" ] ; then
            logger -t "SmartDNS" "Startup failed..."
            logger -t "SmartDNS" "delete"$smartdns_Conf"Add the middle conf-file to the advertisement setting, start again..."
            logger -t "SmartDNS" "If the activation is successful, please check whether the relevant advertising rule format meets the requirements of SmartDNS..."
    ...
    if [ "$smartdns_process"x = x ] ; then
        logger -t "SmartDNS" "Startup failed..."
        logger -t "SmartDNS" "Disable SmartDNS! Please check its port configuration and custom settings ！！！"
        logger -t "SmartDNS" "Restore DNSmasq to provide DNS resolution..."
        nvram set sdns_enable=0
        sdns_enable=0
        action="stop"
        Stop_smartdns
        ...
        logger -t "SmartDNS" "process started..."
    fi
}
...
Stop_smartdns () {
    ...
    logger -t "SmartDNS" "End the SmartDNS process..."
    Change_adbyby
    Change_dnsmasq
    Change_iptable
    ...
    if [ "$smartdns_process"x = x ] && [ "$sdns_enable" = 0 ] ; then 
        rm  -f "$smartdns_Ini"
        logger -t "SmartDNS" "server disabled..."
    fi
}
...
# [Call each sub-function]
    ...
    start)
        ...
        Start_smartdns
        logger -t "SmartDNS" "server started..."
        sleep 2
    ...
    stop)
        ...
        if [ "$smartdns_process"x != x ] ; then
            case $sdns_enable in
            0)
                logger -t "SmartDNS" "deactivate server ..."
                ;;
            1)
                logger -t "SmartDNS" "restart server..."
                ;;
            esac
        fi
        Stop_smartdns
    ...
    restart)
        ...
        Check_ss
        Start_smartdns
        logger -t "SmartDNS" "The server has restarted.."
        sleep 2
        echo 3 > /proc/sys/vm/drop_caches
        ;;
    reset)
